...
TEST(TestHyperTrie, test_single_write_read) {
    ...
    std::variant<HyperTrie<int> *, int> &safed_value_ = trie.get(key);
...
TEST(TestHyperTrie, test_double_write_read0) {
    ...
    // validate
    variant<HyperTrie<int> *, int> safed_value_ = trie.get(key);
...
TEST(TestHyperTrie, test_mult_write_read1) {
    ...
    for (unsigned int i = 0; i < keys.size(); ++i) {
        ...
        variant<HyperTrie<short> *, short>safed_value_ = trie.get(key);
...
TEST(TestHyperTrie, DISABLED_test_read_empty_key) {
    ...
    variant<HyperTrie<short> *, short> this_trie_ = trie.get(empty_key);
...
TEST(TestHyperTrie, test_multi_level_read) {
    ...
    variant<HyperTrie<short> *, short> subtrie_0_ = trie.get(subkey_0);
    ...
    variant<HyperTrie<short> *, short> subtrie_04_ = subtrie_0->get(subkey_4);
    ...
    variant<HyperTrie<short> *, short> subtrie_048_ = subtrie_04->get(subkey_8);
    ...
    variant<HyperTrie<short> *, short> subtrie_0489_ = subtrie_048->get(subkey_9);
