...
RUN cd /conan_cache && conan install . --build=missing --settings compiler.libcxx="libstdc++11"
...
RUN mkdir /tentris/build && cd /tentris/build && conan install .. --build=missing --settings compiler.libcxx="libstdc++11" && cmake -G "Unix Makefiles" -DCMAKE_BUILD_TYPE=Release .. && make -j tentris_server
