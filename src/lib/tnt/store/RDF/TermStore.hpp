...
template<>
struct std::equal_to<std::shared_ptr<tnt::store::rdf::Term>> {
	bool operator()(const std::shared_ptr<tnt::store::rdf::Term> &lhs,
					const std::shared_ptr<tnt::store::rdf::Term> &rhs) const {
