...
namespace tentris::tensor::hypertrie {
	...
	union Children {
	private:
		using inner_edges = tentris::util::container::VecMap<key_part_t, BoolHyperTrie *>;
		using leaf_edges = tentris::util::container::VecSet<key_part_t>;
	public:
	...
	class BoolHyperTrie {
	private:
		static constexpr int factorial(int n) { return n <= 1 ? 1 : (n * factorial(n - 1)); }
	...
	public:
		...
		getDiagonal(const std::vector<key_pos_t> &key_positions, const key_part_t &key_part) const {
			...
			PosCalc const *pos_calc = PosCalc::getInstance(_depth);
	...
	private:
		...
		inline BoolHyperTrie *getSlice_internal(std::map<key_pos_t, key_part_t> &non_slice_key_parts) const {
			...
			BoolHyperTrie *current_subtrie = const_cast<BoolHyperTrie *>(this);
			PosCalc const *pos_calc = PosCalc::getInstance(this->_depth);
	...
	public:
		...
		template<typename T>
		key_pos_t extractPosOfMinCardKeyPos(T &key_poss, PosCalc const *posCalc) const {
	...
	private:
		void del_rek([[maybe_unused]] const Key_t &key,
					 [[maybe_unused]] tsl::hopscotch_map<subkey_mask_t, BoolHyperTrie *> &finished_subtries,
					 [[maybe_unused]] PosCalc const *pos_calc) {
		 ...
		 */
		void set_rek(const Key_t &key, tsl::hopscotch_map<subkey_mask_t, BoolHyperTrie *> &finished_subtries,
		...
		void set(const Key_t &key, const bool &value) {
			...
			if (get(key)) {
				if (not value) {
					...
					tsl::hopscotch_map<subkey_mask_t, BoolHyperTrie *> finished_subtries(factorial(_depth)*2); // TODO: inizialize here
				...
				if (value) {
					// cache for already created sub HyperTries.
					tsl::hopscotch_map<subkey_mask_t, BoolHyperTrie *> finished_subtries(factorial(_depth)*2);
...
//        }
		...
		friend struct fmt::formatter<tentris::tensor::hypertrie::BoolHyperTrie>;
	...
	public:
		...
		public:
			friend struct fmt::formatter<tentris::tensor::hypertrie::BoolHyperTrie::DiagonalView>;
		...
		public:
			...
			DiagonalView(const BoolHyperTrie *trie, const std::vector<key_pos_t> &positions) :
				...
				if (trie->depth() == 1) {
					...
					_max_ind = _inner_edges->size();

					// calculate the positions of the key to use
					PosCalc const *pos_calc = PosCalc::getInstance(trie->depth())->use(min_key_pos);
		...
		private:
			static key_part_t first_from_lower_bound_I(DiagonalView &view, const key_part_t &lower) {
				if (lower != view._leaf_edges->keys().at(view._min_ind)) {
					view._min_ind = tentris::util::container::insert_pos(view._leaf_edges->keys(), view._min_ind,
																	 view._max_ind, lower);
			...
			static key_part_t first_from_lower_bound_II(DiagonalView &view, const key_part_t &lower) {
				...
				if (lower != children_keys.at(view._min_ind)) {
					view._min_ind = tentris::util::container::insert_pos(children_keys, view._min_ind,
																	 view._max_ind, lower);
				}
				return increment_current_until_hit_II(view);
			}

			static key_part_t increment_current_until_hit_II(DiagonalView &view) {
				const std::vector<key_part_t> &children_keys = view._inner_edges->keys();
				const std::vector<BoolHyperTrie *> &children_values = view._inner_edges->values();
				key_part_t current_key_part;
				while (view._min_ind < view._max_ind) {
			...
			static key_part_t first_from_lower_bound_III(DiagonalView &view, const key_part_t &lower) {
				...
				if (lower != childrens_keys.at(view._min_ind)) {
					view._min_ind = tentris::util::container::insert_pos(childrens_keys, view._min_ind,
																	 view._max_ind, lower);
				}
		...
		private:
			static bool containsAndUpdateLower_I(DiagonalView &view, const key_part_t &key_part) {
				size_t ind = tentris::util::container::insert_pos(view._leaf_edges->keys(), view._min_ind,
															  view._max_ind, key_part);
				if (ind < view._max_ind) {
			...
			static bool containsAndUpdateLower_II(DiagonalView &view, const key_part_t &key_part) {
				...
				size_t ind = (current_key_part == childrens_keys.at(view._min_ind))
							 ? view._min_ind
							 : tentris::util::container::insert_pos(childrens_keys, view._min_ind, view._max_ind, key_part);
			...
			static bool containsAndUpdateLower_III(DiagonalView &view, const key_part_t &key_part) {
				...
				size_t ind = (current_key_part == childrens_keys.at(view._min_ind))
							 ? view._min_ind
							 : tentris::util::container::insert_pos(childrens_keys, view._min_ind, view._max_ind,
																key_part);
		...
		private:
			...
			static void setMinMax_I(DiagonalView &view, key_part_t &min, key_part_t &max) {
				...
				// calc max
				if (view._leaf_edges->keyByInd(view._max_ind - 1) != max) {
					const size_t ind = tentris::util::container::insert_pos(view._leaf_edges->keys(), view._min_ind,
																		view._max_ind, max);
					if (view._min_ind > ind) {
						view._size = 0;
						return;
					} else if (ind < view._max_ind) {
						view._max_ind = ind + 1;
						view._max = view._leaf_edges->keyByInd(view._max_ind - 1);
						max = view._max;
					}
				}
				if (min <= max) {
			...
			static void setMinMax_III(DiagonalView &view, key_part_t &min, key_part_t &max) {
				...
				if (view._inner_edges->keyByInd(view._min_ind) != min) {
					view._min_ind = tentris::util::container::insert_pos(view._inner_edges->keys(), view._min_ind,
																	 view._max_ind, min);
					if (view._min_ind >= view._max_ind) {
						view._size = 0;
						return;
				...
				// calc max
				if (view._inner_edges->keyByInd(view._max_ind - 1) != max) {
					const size_t ind = tentris::util::container::insert_pos(view._inner_edges->keys(), view._min_ind,
																		view._max_ind, max);
					if (view._min_ind > ind) {
						view._size = 0;
						return;
						...
						view._max_ind = ind + 1;
						view._max = view._inner_edges->keyByInd(view._max_ind - 1);
						max = view._max;
					}
				}
				if (min <= max) {
...
template<>
struct fmt::formatter<tentris::tensor::hypertrie::BoolHyperTrie> {
	template<typename ParseContext>
	constexpr auto parse(ParseContext &ctx) { return ctx.begin(); }

	template<typename FormatContext>
	auto format(const tentris::tensor::hypertrie::BoolHyperTrie &p, FormatContext &ctx) {
...
template<>
struct fmt::formatter<tentris::tensor::hypertrie::BoolHyperTrie::DiagonalView> {
	template<typename ParseContext>
	constexpr auto parse(ParseContext &ctx) { return ctx.begin(); }

	template<typename FormatContext>
	auto format(const tentris::tensor::hypertrie::BoolHyperTrie::DiagonalView &p, FormatContext &ctx) {
		return format_to(ctx.begin(),
						 "<BoolHyperTrie>"
						 " _positions: {}"
						 " _min:   {}"
