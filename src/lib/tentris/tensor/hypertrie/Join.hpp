...
namespace tentris::tensor::hypertrie {
    ...
    public:
        ...
        friend struct fmt::formatter<tentris::tensor::hypertrie::Join>;
        ...
        public:
            ...
            iterator(Join &join, bool ended = false) :
                ...
                if (not _ended) {
                    if (_current_key_part <= _last_key_part) {
                        ...
                        const std::vector<size_t> _sort_order = tentris::util::container::sortPermutation(
                        ...
                        ::tentris::util::container::applyPermutation(_diagonals, _sort_order);
                        const std::vector<size_t> &inv = ::tentris::util::container::invPermutation(_sort_order);
            ...
            friend struct fmt::formatter<tentris::tensor::hypertrie::Join::iterator>;
...
template<>
struct fmt::formatter<tentris::tensor::hypertrie::Join> {
    template<typename ParseContext>
    constexpr auto parse(ParseContext &ctx) { return ctx.begin(); }

    template<typename FormatContext>
    auto format(const tentris::tensor::hypertrie::Join &p, FormatContext &ctx) {
...
template<>
struct fmt::formatter<tentris::tensor::hypertrie::Join::iterator> {
    template<typename ParseContext>
    constexpr auto parse(ParseContext &ctx) { return ctx.begin(); }

    template<typename FormatContext>
    auto format(const tentris::tensor::hypertrie::Join::iterator &p, FormatContext &ctx) {
        return format_to(ctx.begin(),
                         "<Join::iterator>"
                         " _current_key_part: {}"
                         " _last_key_part:    {}"
