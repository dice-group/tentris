...
namespace tentris::store::cache {
	...
	struct GraphqlExecutionPackage {
        ...
        using GraphqlDocument = graphql::GraphqlDocument;
	...
	public:
         ...
         */
		explicit GraphqlExecutionPackage(std::shared_ptr<GraphqlDocument>& document,
										 const std::string& query_name = "") : query_name(query_name) {
            using namespace logging;
            logDebug(fmt::format("Parsing query: {}", query_name));
            ParsedGraphql parsed_graphql{document->getQuery(query_name), AtomicGraphqlSchema::getInstance()};
		...
		[[nodiscard]] std::vector<std::shared_ptr<Einsum>> generateEinsums(const time_point_t &timeout) {
			...
			for(auto i : iter::range(subscripts.size())) {
				auto einsum = std::make_shared<Einsum>(subscripts[i], operands[i], timeout);
