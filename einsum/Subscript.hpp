...
private:
    ...
    void init(vector<vector<label_t>> &raw_operand_subscripts, vector<label_t> &raw_result_subscript) {
        ...
        for (op_pos_t op_pos = 0; op_pos < operand_subscripts.size(); ++op_pos) {
            operand_labels[op_pos] = operand_subscripts[op_pos];
        }
...
private:
    ...
    map<op_pos_t, vector<label_t>> operand_labels{};
    vector<label_t> result_labels{};
    unordered_set<label_t> distinct_operand_labels{};
    ...
    unordered_map<label_t, op_pos_t> operands_with_label{};
